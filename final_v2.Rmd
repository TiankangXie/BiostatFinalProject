...
for(i in 1:3){
  for(j in 1:3){
    for(k in 1:3){
      ...
      #print(j)
      #print(k)
      store_hospital = dataset_tmp[sample(length(dataset_tmp), size=6, replace =F)]
      #In  total we sampled 405 hospitals
      new_frame = NIS[which(NIS$HOSP_NIS.x %in% store_hospital),]
      new_frame2 = new_frame[sample(nrow(new_frame), size = round(0.33*nrow(new_frame)), replace = F),]
...
m <- glmer(SEVERITY ~ (1 + AGE | Census_Division/HOSP_NIS.x) + AWEEKEND + (Control |Census_Division) + (LocTeach |Census_Division) + (Bedize |Census_Division) + (TOTAL_DISC|Census_Division), data =NIS_short,family = binomial(link = "logit"))
